...
function onCancel() {
  throw Error(t2("error.userCancel"));
}
function onError(str) {
  throw Error(t2("signal.{str}", { str }));
}
function isValidPath(path3) {
  return !!path3 && path3.length > 0;
}
function isValidPackageName(name) {
  return /^[a-zA-Z_][a-zA-Z0-9_]*(\.[a-zA-Z_][a-zA-Z0-9_])*$/.test(name);
}
function accumulate(accumulator, times, callback) {
...
var genVoMapperCommand = new Command().name("genVoMapper").requiredOption("--project-root <path>", "\u9879\u76EE\u6839\u76EE\u5F55").option("--package-name <name>", "\u9879\u76EE\u6839\u76EE\u5F55", "com.github.alphafoxz.oneboot").requiredOption("--domain-module <name>", "\u9886\u57DF\u6A21\u5757\u540D").requiredOption("--output-module <name>", "\u8F93\u51FA\u6A21\u5757\u540D").action((options) => {
  currentCommand.value = "genVoMapper" /* GenVoMapper */;
...
var startCommand = new Command().name("start");
var program2 = new Command().name("oneboot-tool").addCommand(startCommand).addCommand(genVoMapperCommand);
var currentCommand = ref("none" /* None */);
var genVoMapperArgs = ref({});
function configArgsFromCommandLine() {
  program2.parse(process.argv);
  if ("none" /* None */ !== currentCommand.value) {
    isReady.value = true;
  }
}
configArgsFromCommandLine();
async function configArgsFromUserChoise() {
  const { langurage } = await (0, import_prompts.default)(
    [
      {
        name: "langurage",
        type: "select",
  ...
  lang_default.action.updateLang(langurage);
  const { subcommand } = await (0, import_prompts.default)(
    [
      {
        name: "subcommand",
        type: "select",
        message: t3("question.subcommand"),
        choices: [
          {
            title: t3("question.subcommand.generateVoMapper"),
            value: "genVoMapper" /* GenVoMapper */
          }
        ]
      }
    ],
...
async function configGenVoMapperFromUserChoise() {
  ...
  const { projectRoot } = await (0, import_prompts.default)(
    [
      {
        name: "projectRoot",
        type: "text",
        message: t3("question.projectRoot"),
        initial: `${defaultProjectRoot}`,
        onState: (state) => {
          if (!isValidPath(state.value)) {
            onError(t3("error.badArgs"));
          }
          return state.value;
        }
      },
      {
        name: "packageName",
        type: "text",
        message: t3("question.packageName"),
  ...
  if (!import_node_fs.default.existsSync(projectRoot) || !import_node_fs.default.statSync(projectRoot).isDirectory()) {
    onError(t3("error.shouldBeValidDir{dir}", { dir: projectRoot }));
  }
  const projectChildren = [];
  import_node_fs.default.readdirSync(projectRoot).forEach((i) => {
    if (!i.startsWith(".") && !i.startsWith("_") && import_node_fs.default.statSync(import_node_path.default.join(projectRoot, i)).isDirectory()) {
      projectChildren.push({ title: i, value: i });
    }
  });
  const { domainModule, outputModule } = await (0, import_prompts.default)(
    [
      {
        name: "domainModule",
        type: "select",
        message: t3("question.domainModule"),
        choices: projectChildren
      },
      {
        name: "outputModule",
        type: "select",
        message: t3("question.outputModule"),
        choices: projectChildren
      }
    ],
    { onCancel }
...
((java2) => {
  let Grammar;
  ((Grammar2) => {
    Grammar2["package_declaration"] = "package_declaration";
    Grammar2["import_declaration"] = "import_declaration";
    Grammar2["scoped_identifier"] = "scoped_identifier";
    Grammar2["class_declaration"] = "class_declaration";
    Grammar2["interface_declaration"] = "interface_declaration";
    Grammar2["enum_declaration"] = "enum_declaration";
    Grammar2["record_declaration"] = "record_declaration";
    Grammar2["type_identifier"] = "type_identifier";
    Grammar2["identifier"] = "identifier";
    Grammar2["formal_parameters"] = "formal_parameters";
    Grammar2["formal_parameter"] = "formal_parameter";
  })(Grammar || (Grammar = {}));
  const parser = new import_tree_sitter.default();
  parser.setLanguage(import_tree_sitter_java.default);
  function parseTree(content) {
    return parser.parse(content);
  }
  java2.parseTree = parseTree;
  function parse(path3, content) {
    const result = {
      _filePath: path3,
      package_declaration: void 0,
      import_declaration: [],
      class_declaration: [],
    ...
    for (const child of children) {
      if (child.type === "package_declaration" /* package_declaration */) {
        result.package_declaration = parsePackageMeta(child, content);
      } else if (child.type === "import_declaration" /* import_declaration */) {
        result.import_declaration.push(findByGrammar(child, "scoped_identifier" /* scoped_identifier */, content));
      } else if (child.type === "class_declaration" /* class_declaration */) {
        result.class_declaration.push(parseClassMeta(child, content));
      } else if (child.type === "interface_declaration" /* interface_declaration */) {
        result.interface_declaration.push(parseInterfaceMeta(child, content));
      } else if (child.type === "record_declaration" /* record_declaration */) {
  ...
  java2.parse = parse;
  function parsePackageMeta(node, content) {
    const child = node.children.find((ele) => ele.type === "scoped_identifier" /* scoped_identifier */);
    const result = {
      grammar: "package_declaration" /* package_declaration */,
      name: content.substring(child.startIndex, child.endIndex)
    };
    return result;
  }
  function parseClassMeta(node, content) {
    const result = {
      grammar: "class_declaration" /* class_declaration */,
  ...
  function parseInterfaceMeta(node, content) {
    const result = {
      grammar: "interface_declaration" /* interface_declaration */,
      name: findName(node, content),
      staticFields: [],
      fields: []
    };
  ...
  function parseRecordMeta(node, content) {
    const result = {
      grammar: "record_declaration" /* record_declaration */,
      name: findName(node, content),
      formalParameters: []
    };
    for (const child of node.children) {
      if (child.type === "formal_parameters" /* formal_parameters */) {
        ...
        for (const formalParameter of formalParameters) {
          result.formalParameters.push({
            grammar: "formal_parameter" /* formal_parameter */,
  ...
  function parseFormalParameters(node, content) {
    const result = [];
    node.children.forEach((e) => {
      if (e.type !== "formal_parameter" /* formal_parameter */) {
        return;
      }
      result.push({
        grammar: "formal_parameter" /* formal_parameter */,
        // type: findName(e, content),
        type: findByGrammar(e, "type_identifier" /* type_identifier */, content),
        name: findName(e, content)
      });
    });
    return result;
  }
  function findName(node, content) {
  ...
  function findByGrammar(node, grammar, content) {
    for (const child of node.children) {
...
// src/gen-vo-mapper.ts
var import_node_fs2 = __toESM(require("node:fs"), 1);
var import_node_path2 = __toESM(require("node:path"), 1);
var argsStore = useArgs();
async function gen_vo_mapper_default() {
  await argsStore.action.init();
  const param = argsStore.state.genVoMapperArgs.value;
  const parseResult = walkJavaFile(import_node_path2.default.join(param.projectRoot, param.domainModule));
  let parseMap = {};
  parseMap = parseResult.reduce((map, currentValue) => {
    const regStr = `^${import_node_path2.default.join(
      param.projectRoot,
      param.domainModule,
      "src",
      "main",
      "java",
      param.packageName.replace(/\./g, import_node_path2.default.sep),
      param.domainModule.replace(/-/g, import_node_path2.default.sep)
    )}${import_node_path2.default.sep}([a-zA-Z0-9_]+)${import_node_path2.default.sep}vo.*$`;
...
function walkJavaFile(parentPath, parseResult = []) {
  const files = import_node_fs2.default.readdirSync(parentPath);
  for (const file of files) {
    const filePath = import_node_path2.default.join(parentPath, file);
    ...
    parseResult.push(java.parse(filePath, content));
  }
  return parseResult;
}
function genCode(projectRoot, packageName, outputModule, parseMap) {
  const files = [];
  Object.keys(parseMap).forEach((key) => {
    ...
    for (const vo of vos) {
      ...
      vo.record_declaration.forEach((record) => {
        ...
        voMapperCode.addImport(((_a = vo.package_declaration) == null ? void 0 : _a.name) + "." + vo.record_declaration[0].name);
    ...
    files.push({
      filePath: import_node_path2.default.join(
        ...
        "main",
        "java",
        packageName.replace(/\./g, import_node_path2.default.sep),
        outputModule.replace(/-/g, import_node_path2.default.sep),
        "gen",
        "mapper",
...
var VoMapperCode = class {
  packageName;
  ClassName;
  importPackages = /* @__PURE__ */ new Set();
  mappings = {};
  ...
  addImports(packageNames) {
    this.importPackages = /* @__PURE__ */ new Set([...this.importPackages, ...packageNames]);
  }
  addMapping(voName, mapping) {
...
`;
    ...
    Object.keys(this.mappings).forEach((voName) => {
      ...
      const args = accumulate([], fieldsSize, (accumulator, i) => {
        if (i === fieldIndex) {
...
function writeFile(...files) {
  for (const file of files) {
    if (!import_node_fs2.default.existsSync(file.filePath)) {
      import_node_fs2.default.mkdirSync(import_node_path2.default.dirname(file.filePath), { recursive: true });
    }
    import_node_fs2.default.writeFileSync(file.filePath, file.content);
  }
}

// src/index.ts
var t4 = lang_default.action.t;
console.info(t4("signal.scriptStart"));
var argsStore2 = useArgs();
start().then(() => {
  console.info(t4("signal.successComplete"));
}).catch((e) => {
  console.error(e.message);
  console.error(t4("signal.exitWithError"));
});
process.on("SIGINT", onCancel);
async function start() {
  await argsStore2.action.init();
  switch (argsStore2.state.currentCommand.value) {
    case "genVoMapper":
      gen_vo_mapper_default();
      break;
    case "none":
      break;
  }
